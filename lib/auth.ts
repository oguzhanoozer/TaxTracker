...
export async function getCurrentUser(): Promise<User> {
  // Self-hosted mode — auto-create the local user, no welcome screen.
  if (config.selfHosted.isEnabled) {
    const { getOrCreateSelfHostedUser } = await import("@/models/users")
    return await getOrCreateSelfHostedUser()
  }

  // Cloud mode — try the session first.
  ...
  // No session — fall back to the local user so the app never bounces visitors
  // to a login screen. Portfolio demo: everyone gets the same playground.
  const { getOrCreateSelfHostedUser } = await import("@/models/users")
  return await getOrCreateSelfHostedUser()
}
