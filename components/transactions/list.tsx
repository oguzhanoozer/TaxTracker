  ...
  total: {
    ...
    formatValue: (transaction: Transaction) => (
      <div className="text-right text-lg">
        <div
          className={cn(
            { income: "text-emerald-600", expense: "text-red-500", other: "text-gray-800" }[transaction.type || "other"],
        ...
        <TableBody>
          {transactions.map((transaction) => (
            <TableRow
              key={transaction.id}
              className={cn(
                isTransactionIncomplete(fields, transaction) && "bg-amber-50",
                selectedIds.includes(transaction.id) && "bg-gray-100",
