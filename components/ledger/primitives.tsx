...
/* ============================================================
   Ledger primitives — TSX port of new-design/components.jsx
   ============================================================ */

/* ---- Icon paths (Lucide-ish) ---- */
const PATHS: Record<string, string> = {
  ...
  settings:
    "M12 15a3 3 0 100-6 3 3 0 000 6zM19.4 13a7.9 7.9 0 000-2l2-1.5-2-3.4-2.4 1a8 8 0 00-1.7-1l-.3-2.6H9.3L9 4.6a8 8 0 00-1.7 1l-2.4-1-2 3.4L3 11a7.9 7.9 0 000 2l-2 1.5 2 3.4 2.4-1a8 8 0 001.7 1l.3 2.6h4.6l.3-2.6a8 8 0 001.7-1l2.4 1 2-3.4L19.4 13z",
  ...
  paperclip:
    "M21 11l-8.5 8.5a4 4 0 01-6-6L13 6a2.5 2.5 0 014 3l-7 7a1 1 0 01-1.5-1.5l6.5-6.5",
  calendar:
    "M7 3v3M17 3v3M4 8h16M5 6h14a1 1 0 011 1v13a1 1 0 01-1 1H5a1 1 0 01-1-1V7a1 1 0 011-1z",
  ...
  coffee:
    "M3 8h13v5a5 5 0 01-5 5H8a5 5 0 01-5-5V8zM16 9h2.5a2.5 2.5 0 010 5H16M6 2v2M10 2v2",
...
/* ---- Button ---- */
type BtnVariant = "primary" | "default" | "ghost" | "soft" | "danger"
type BtnSize = "sm" | "md" | "lg"

const BTN_SIZES: Record<BtnSize, { padding: string; fontSize: number; gap: number; h: number }> = {
...
/* ---- Pill ---- */
export type PillTone = "neutral" | "blue" | "green" | "amber" | "red" | "accent"
...
export function Pill({
  ...
  const [fg, bg] = PILL_MAP[tone] || PILL_MAP.neutral
  return (
    <span
      style={{
        display: "inline-flex",
...
/* ---- Category chip (color comes from prop) ---- */
...
/* ---- ScanStatus ---- */
type ScanState = "queued" | "reading" | "done" | "failed"

export function ScanStatus({ status }: { status: ScanState }) {
  const map: Record<ScanState, { tone: PillTone; label: string; dot: boolean | "pulse" }> = {
    queued: { tone: "neutral", label: "Queued", dot: false },
    reading: { tone: "blue", label: "Reading…", dot: "pulse" },
    done: { tone: "green", label: "Done", dot: true },
    failed: { tone: "red", label: "Failed", dot: false },
  }
...
export function ReceiptThumb({
  ...
  return (
    ...
    >
      ...
      <Icon name={kind === "pdf" ? "file" : "eye"} size={15} style={{ position: "relative" }} />
    </div>
  )
}

/* ---- Pulse animation reused by inline graphics ---- */
export function PulseStyle() {
  return null /* keyframes live in globals.css now */
}

/* Re-export server-safe formatter */
